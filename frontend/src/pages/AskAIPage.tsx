      ...
      <div className="col-12">
        <div className="card">
          ...
          <div className="row mt-3" style={{gap:8}}>
            <input style={{flex:1}} placeholder="Type your question..." value={question} onChange={e=>setQuestion(e.target.value)} />
            <button className="primary" onClick={handleAsk}>{loading ? 'Thinking…' : 'Ask'}</button>
          </div>
          {parsed && (
            <div className="mt-3">
              <div className="badge">Parsed Filters</div>
              <pre className="mono" style={{whiteSpace:'pre-wrap'}}>{JSON.stringify(parsed, null, 2)}</pre>
            </div>
          )}
        </div>
      </div>
